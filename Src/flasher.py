...
import sys, serial, binascii, time, struct, threading, argparse
...
class STM32Flasher(object):
    ...
    def _crc_stm32(self, data):
        ...
        for d in data:
            ...
            for i in range(32):
                if crc & msb_mask:
                    crc = ((crc << 1) ^ poly) & mask #Polynomial used in STM32
                else:
                    crc = (crc << 1) & mask
        # print('crc', f'{crc:#010x}', f'{crc:032b}')

        return (crc & mask)

    def _create_cmd_message(self, msg):
        #Encodes a command adding the CRC32
        crc_value = self._crc_stm32(msg)
        s = struct.pack("<I", crc_value)
        cmd = list(msg) + list(s)
        return cmd
    ...
    def _encryptMessage(self, data):
        ...
        #adds the CRC-32
        data = AESEncryptor.encrypt(bytes(data))
    ...
    def getID(self):
        ...
        data = self.serial.read(1)
        if len(data) == 1:
            recv_data = struct.unpack("b", data)
            if recv_data[0] != ACK:
...
if __name__ == '__main__':
    ...
    t = threading.Thread(target=doErase, args=(None,))
    t.start()
